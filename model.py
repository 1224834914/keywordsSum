...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    if FLAGS.pointer_gen:
      ...
      self._keywords_batch_extend_vocab = tf.placeholder(tf.int32, [hps.batch_size, hps.keyword_num],name='keywords_batch_extend_vocab')
    #keywords
    self._keywords_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.keyword_num],name='keywords_batch')
    # decoder part
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    feed_dict[self._keywords_batch] = batch.keywords_batch
    if FLAGS.pointer_gen:
      feed_dict[self._enc_batch_extend_vocab] = batch.enc_batch_extend_vocab
      feed_dict[self._max_art_oovs] = batch.max_art_oovs
      feed_dict[self._keywords_batch_extend_vocab] = batch.keywords_batch_extend_vocab
    if not just_enc:
  ...
  def _add_decoder(self, inputs):
    ...
    outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state, self._enc_states, self._enc_padding_mask, self._keywords, cell, initial_state_attention=(hps.mode=="decode"), pointer_gen=hps.pointer_gen, use_coverage=hps.coverage, prev_coverage=prev_coverage)
  ...
  def _add_seq2seq(self):
    ...
    hps = self._hps
    vsize = self._vocab.size() # size of the vocabulary 50000
    ...
    with tf.variable_scope('seq2seq'):
      ...
      # Add embedding matrix (shared by the encoder and decoder inputs)  embedding[50000,128]
      with tf.variable_scope('embedding'):
        #词表embedding,50000个词，每个词的维度是128
        embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
        
        if hps.mode=="train": self._add_emb_vis(embedding) # add to tensorboard
        
        #embedding_lookup(params,ids)按照ids顺序返回params中的第ids行
        emb_enc_inputs = tf.nn.embedding_lookup(embedding, self._enc_batch) # tensor with shape (16, 1500, 128)
        emb_dec_inputs = [tf.nn.embedding_lookup(embedding, x) for x in tf.unstack(self._dec_batch, axis=1)] # list 40 tensor with shape(16, 128)
        emb_keywords_inputs = tf.reduce_sum(tf.nn.embedding_lookup(embedding, self._keywords_batch),[1])#[16,128]

      # Add the encoder.
      enc_outputs, fw_st, bw_st = self._add_encoder(emb_enc_inputs, self._enc_lens)
      self._enc_states = enc_outputs

      self._keywords = emb_keywords_inputs
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    feed[self._keywords_batch] = batch.keywords_batch
    ...
    if FLAGS.pointer_gen:
      ...
      feed[self._keywords_batch_extend_vocab] = batch.keywords_batch_extend_vocab
